...
class ArsipStatisController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            $data = Document::with('category','label')->where('labels_id',4)->get();
            return DataTables::of($data)

                ->addIndexColumn()
                ->addColumn('action', function ($data) {
                    $btn = ' <form class="inline-block" action="' . route('statis.show', $data->id) . '" method="GET">
                    ...
                    $btn .= ' <form class="inline-block" action="' . route('statis.edit', $data->id) . '" method="GET">
                    <button  class="border border-gray-500 bg-white-500 text-black rounded-md px-2 py-1 m-2 transition duration-500 ease select-none hover:bg-green-500 focus:outline-none focus:shadow-outline" >
                           Edit
                       </button>

                   </form>';
                    $btn .= ' <form class="inline-block" action="' . route('statis.destroy', $data->id) . '" method="POST">
